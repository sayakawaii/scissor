...
- [x] Token-based budgeting instead of character counts. Context and compaction
  budgets are expressed in tokens via a single `estimateTokens()` seam with a
  per-provider chars-per-token ratio (`packages/core/src/tokens.ts`), so a real
  tokenizer can drop in without touching the loop. Character-based callers are
  converted at the boundary. Still open: a *per-turn* budget so a single burst of
  retrieval/tool output can't blow the window.
- [ ] Relevance-aware trimming: keep the messages most relevant to the current
  request (e.g. the files it touches) rather than strictly the most recent. The
  hard fallback is now max-min fair-share allocation across messages
  (`packages/core/src/truncate.ts`) rather than dropping whole rounds, which
  removes the worst case, but the allocation is still size-based, not
  relevance-based.
- [x] Tool-output spilling: results over budget are written to
  `.scissor/tool-output/<callId>.txt` by `createOutputSpillGuard`, and the model
  gets head + tail + path + a do-not-retry notice. Applies uniformly to every
  tool including MCP ones. Still open: *summarizing* (rather than eliding) large
  results once they are no longer the focus.
...
## 6b. Sandbox / isolation

Shipped: an always-on write-protection denylist enforced for every tool
(`packages/core/src/sandbox/policy.ts`), a fail-closed command classifier that
canonicalizes shell quoting before matching (`sandbox/command.ts`),
`required_permissions` escalation on `run_shell` with the policy described in the
system prompt, and Docker/WSL backends that refuse rather than silently downgrade
(`sandbox/backend.ts`). Remaining:

- [ ] Native backends: seatbelt (`sandbox-exec`) on macOS and Landlock on Linux.
  Cheaper than a container and no image to maintain, but both need per-platform
  profile generation from the same policy object.
- [ ] Long-lived container reuse. Today each command gets a fresh `--rm`
  container, which costs a startup per call. The ownership + schema-version
  labels are already in place for a persistent container; what's missing is the
  health gate and lifecycle management around it.
- [ ] Read restrictions are declared in the policy (`additionalReadPaths`,
  `additionalReadonlyPaths`) but only the Docker mount set enforces them; the
  in-process file tools still allow reads anywhere.
- [ ] Network policy under WSL. WSL shares the host network stack, so
  `network: "none"` cannot be honored there — currently reported honestly rather
  than enforced. A per-distro netns would fix it.
- [ ] Validate the full verification loop (`npm run check`, `diagnostics`) inside
  each backend against a few real project types; the toolchain has to be
  reachable inside the sandbox or the loop silently degrades.
...
- [~] **(D) Real-codebase task set (Databricks-faithful)**: curate a harder
  ...
  - [x] **Scheme D — realistically-shaped task**: `tlv-uint40-decode-bug`. A
    variable-width binary decoder where most widths use `binary.BigEndian`, but
    the 40-bit path (`Uint40`) is hand-rolled with explicit shifts; here the
    5-byte decoder scales the MSB by 2**32→2**24, so it's only wrong once the
    value ≥ 2**32 (small inputs and all other widths pass). Buried in a
    codec-flavoured tree with correct look-alike
  ...
  - [x] **Live signal through a restrictive network.** Where a network blocks
    `api.deepseek.com` directly but allows `CONNECT` to a reachable host, scissor
    is now proxy-aware (`NODE_USE_ENV_PROXY=1` → provider uses Node's global
    fetch; `providers/proxy.ts`) and works through a local CONNECT proxy. See the
    §7e table + `docs/benchmarking.md`.
  - [x] **Scheme B — real `go test` bug-fix (`go-uint40-decode-bug`).** A
    self-contained, stdlib-only Go module with the same 40-bit decode bug,
    scored by an actual `go test` red→green (via an
  ...
  - [x] **Scheme C — verify-loop ablation on the Go task.** `go build ./...;
    ...
    - [x] **Live signal (B) obtained**: `go-uint40-decode-bug`
  ...
  - [x] **Schema clamp bug (`tlv-attr-slice-panic`).** Pre-fix code slices a
    schema field to its declared size without clamping to the received payload →
    slice-out-of-range panic on short payloads (record types only present in the
    default schema). Packaged self-contained (stdlib-only Go) so a
  ...
  - [ ] **Automate the distillation (`eval-gen --from-commit`).** Today `eval-gen`
    ...
    become tasks without hand-authoring. Mining tips: prefer single-file logic
    fixes with a clear symptom in the commit message (`git log --grep=fix`).
...
- [x] **Phase 0 — measure scissor's current over-reading.** The eval harness now
  ...
  annotates the minimum-sufficient scope; the real tasks (`go-*`,
  `buried-bug-fix`, `deep-median-bug`, `tlv-uint40-decode-bug`) are annotated
  ...
  **files/task column + ACRR** to the matrix. This answers "does scissor over-read
  ...
  - [x] **Live signal obtained** (deepseek-chat, router off,
    ...
    | retrieval QA ×8 | 42→19 | 11.8k→13.3k (1.12×) | 1.0→0.8 | 0.00→−0.25 |
    | tlv-uint40 (buried bug) | 7→5 | 13.1k→25.2k (1.93×) | 3.0→4.0 | 2.00→3.00 |
  ...
  - [ ] **Metric caveat — `retrieve` under-counts.** `inspectedFiles` counts only
    ...
    file content without a `read_file` call (hence the −0.25 ACRR on retrieval QA: scissor
...
## 10. Voice I/O (spoken conversation via `/voice`)

Talk to scissor: transcribe speech → prompt, and optionally read replies aloud
(e.g. scissor as a mock interviewer). UI-only feature under
`packages/cli/src/voice/`; `core` stays UI-agnostic (only a data-only
`VoiceConfig` in `config.ts`). See `docs/voice.md`.

- [x] **Phase 1 — turn-based local voice.** `SttProvider`/`TtsProvider`
  interfaces; a Windows engine (`System.Speech.Recognition` dictation + SAPI
  synthesis) driven through a temp PowerShell script (same pattern as the Go
  bridge), offline and dependency-free. `resolveVoiceEngine` picks it on win32
  and degrades gracefully elsewhere. `/voice` (one turn), `/voice on|off`
  (continuous, ends on a spoken "stop"), `/voice say`, `/voice status`; replies
  are stripped of code/markdown and length-capped before TTS. The REPL turn body
  was extracted (`runAgentTurn`) so spoken and typed input share one path.
  Deterministic test `scripts/test-voice.mts` (scripted engine — no mic/PS). The
  interviewer use case works today: `/voice` + an opening persona instruction.
  Live mic + `System.Speech` is a manual Windows smoke test (docs/voice.md).
- [x] **Phase 1.1 — language + discoverability fixes** (from first real use).
  Two problems surfaced: (a) non-English speech came back as confident English
  nonsense, because an unset locale silently used the machine-default `en-US`
  recognizer even though a `zh-CN` one was installed; (b) bare `/voice` was a
  single turn, which nobody expects from "voice mode". Now: the recognize script
  reports the culture that actually decoded (`@@ENGINE@@`) and the controller
  warns when it isn't the requested one; `recognizersScript` enumerates installed
  languages (`@@LOCALE@@`) so `/voice status` shows them; `/voice lang <locale>`
  switches at runtime (warning when no recognizer is installed for it); bare
  `/voice` now starts a continuous conversation and `/voice once` is the
  single-turn escape hatch.
- [x] **Phase 2a — persistent local recognizer (no new deps, no API key).** The
  dominant latency was *process + engine startup per turn* (constructing a
  `SpeechRecognitionEngine` costs ~3s), not the model. `win-host.ts` now runs one
  long-lived host — a PowerShell shim that `Add-Type`-compiles a small C# class
  (PowerShell's `Register-ObjectEvent` actions don't fire while the main thread
  blocks on stdin, .NET events do) — in `RecognizeAsync(RecognizeMode.Multiple)`,
  streaming `@@READY@@` / `@@PARTIAL@@` / `@@FINAL@@` / `@@ERR@@` over stdout and
  accepting `PAUSE`/`RESUME`/`QUIT` on stdin. Result: turns start instantly after
  a one-time ~3s startup, live hypotheses render as you speak, and the mic is
  muted around TTS so the agent never transcribes itself. `auto` uses it with an
  automatic `FallbackSttProvider` to the spawn-per-turn engine, and the host is
  disposed on every REPL exit path. Verified end-to-end on Windows with
  `scripts/smoke-voice-host.mts` (`READY in 2826ms — culture zh-CN`); speech
  content itself still needs a human at the mic.
- [ ] **Phase 2a-next — barge-in.** The `PAUSE`/`RESUME` channel is in place, so
  interrupting the agent mid-reply by speaking is now mostly a UI/policy question
  (cancel the TTS + the in-flight turn on a confident partial).
- [ ] **Phase 2c — OneCore DNN engine (accuracy, not latency).** This machine has
  the modern DNN models (`MS-2052-110-WINMO-DNN`) installed, which are far more
  accurate than the legacy desktop recognizers `System.Speech` uses — especially
  for Mandarin. Reaching them needs the WinRT `Windows.Media.SpeechRecognition`
  API (awkward to call from PowerShell/C# interop) instead of `System.Speech`.
  The Phase 2a host protocol (READY/PARTIAL/FINAL over stdout) is engine-agnostic
  and would be reused as-is; only the host body changes. Scope/complexity to be
  reviewed before committing to it.
- [ ] **Phase 2b — cloud streaming STT (blocked on prerequisites).** Deepgram /
  Azure Speech / OpenAI Realtime over WebSocket, with partial hypotheses + VAD
  endpointing and streaming TTS that starts speaking before the reply completes.
  Node's global `WebSocket` is available (no `ws` dep needed) and
  `providers/proxy.ts` can carry it through an HTTP proxy. **Blocked on
  two things the dev machine lacks:** a raw-audio capture binary (no `ffmpeg`/
  `sox` installed — cloud STT needs a PCM stream, unlike the local engine which
  bundles capture+recognition) and a speech API key (only a DeepSeek key is
  configured, and DeepSeek has no speech API).
- [ ] **Phase 3 — cross-platform local + cloud fallback.** macOS (`say` + a local
  whisper.cpp or the dictation API) and Linux; a cloud Whisper batch provider as
  a portable fallback where no OS engine exists.
- [ ] **Optional — interviewer persona mode.** A first-class `--persona` /
  `/persona` that swaps the system prompt and gates coding tools, so "mock
  interview" (and similar role-play) is one flag rather than an opening prompt.
