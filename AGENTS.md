...
- `packages/core` — UI-agnostic engine: providers + router, the agent loop,
 tools, guardrails, prompt/retrieval, edit engine, sandbox (policy + command
 classifier + Docker/WSL backends), MCP client, config + session store. **No
...
- **Cross-cutting tool policy goes in the guardrail pipeline**, not the core loop
 (TDD, approval, oscillation, output spilling are all guardrails). Add new policy
 as a `Guardrail` with `beforeTool`/`afterTool`/`reset`.
- **Safety checks fail closed.** If a command's quoting can't be parsed or a
 requested sandbox backend isn't usable, refuse — never fall through to running
 it. A denied command is reported to the agent as a dead end, not an approval
 prompt, so it stops retrying variations.
- **Coerce tool arguments leniently** via `tools/coerce.ts` (models send `"3"`
 for numbers and stringified arrays), but refuse genuinely ambiguous input rather
 than guessing.
