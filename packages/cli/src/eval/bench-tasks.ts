...
export const BENCH_TASKS: EvalTask[] = [
  ...
  {
    // Requires backgrounding a process: an HTTP server never exits, so this is
    // only solvable if `run_shell` can hand back a running shell (block_until_ms
    // / is_background) and the agent can then query it. A synchronous-only shell
    // tool can do nothing here but block until it is killed.
    id: "background-server",
    title: "Start a server in the background and query it",
    tags: ["write", "shell", "multi-step", "background"],
    prompt:
      "Create server.js (CommonJS, dependency-free) that starts an HTTP server on port 8731 " +
      'and responds to GET /health with the JSON body {"status":"ok"} and content-type ' +
      "application/json. Start the server in the background, then fetch http://localhost:8731/health " +
      "and save the exact response body to health.txt, then shut the server down.",
    async check(dir) {
      if (!(await exists(dir, "server.js"))) return { pass: false, detail: "server.js not created" };
      if (!(await exists(dir, "health.txt"))) {
        return { pass: false, detail: "health.txt not created (server never queried)" };
      }
      const body = (await read(dir, "health.txt")).trim();
      let parsed: unknown;
      try {
        parsed = JSON.parse(body);
      } catch {
        return { pass: false, detail: `health.txt is not JSON: ${JSON.stringify(body.slice(0, 80))}` };
      }
      if ((parsed as { status?: unknown })?.status !== "ok") {
        return { pass: false, detail: `unexpected body: ${JSON.stringify(body.slice(0, 80))}` };
      }
      return { pass: true, detail: "server backgrounded and queried successfully" };
    },
  },
  {
  ...
  {
    // Option D (Scheme D): a variable-width binary decoder where most widths go
    // through a standard big-endian helper, but the 40-bit path is hand-rolled
    // with explicit shifts — exactly the kind of code that invites an off-by-8.
    // Here the 5-byte decoder scales the most-significant byte by 2**24 instead
    // of 2**32, so it's only wrong once b[0] != 0 (value >= 2**32). All the
    // standard widths (1/2/4/8) and small 5-byte values decode correctly, so
    // grep/eyeball isn't enough — the fix needs range/edge reasoning. Buried in a
    // codec-flavoured tree with correct look-alike decoders as red herrings, and
    // probe-scored so neutering check.js can't pass it. (JS bit-shifts are 32-bit,
    // so widths >32 use arithmetic — values stay < 2**53.)
    id: "tlv-uint40-decode-bug",
    title: "Fix a subtle big-endian 40-bit decode bug in a real-flavoured tree",
    tags: ["retrieve", "read", "edit", "debug", "multi-file", "reason", "real"],
    timeoutMs: 300_000,
    oracle: { files: 1 },
    async setup(dir) {
      // Noise mirroring a realistic service layout, so *locating* the decoder matters.
      const areas = [
        "controller",
        "dao",
        "routers",
      ...
      await write(
        dir,
        "src/service/codec/schema/bigendian.js",
      ...
      await write(
        dir,
        "src/service/codec/schema/util.js",
        [
          "// Variable-width big-endian unsigned decode.",
          "function uint40(b) {",
      ...
      await write(
        dir,
        "src/service/codec/schema/index.js",
      ...
      await write(
        ...
        "check.js",
        [
          "const { bytesUInteger } = require('./src/service/codec/schema');",
          "const cases = [",
          "  [[7], 1, 7],",
          "  [[1, 0], 2, 256],",
          "  [[0, 0, 1, 0], 4, 256],",
    ...
    async check(dir) {
      ...
      const src = await read(dir, "src/service/codec/schema/util.js");
...
export function resolveTasks(ids?: string[]): EvalTask[] {
  ...
  // GO_TASKS (Go toolchain) are only ever returned when explicitly named by id
  // — never part of the default (hermetic) suite / pre-push gate.
  return [...EVAL_TASKS, ...BENCH_TASKS, ...GO_TASKS].filter((t) => set.has(t.id));
}
