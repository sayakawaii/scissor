 ...
 *
 * The bug models a variable-width binary decoder: most widths decode via
 ...
 *
 * These tasks need a Go toolchain, so they are reachable only by id via
 * resolveTasks and never enter the hermetic default suite.
 */
import { read, write } from "./task-helpers.js";
import { goAvailable, runGo } from "./go-helpers.js";
import type { EvalTask } from "./tasks.js";

const PKG = "internal/tlvschema";
const GO_MOD = "module tlvdecode\n\ngo 1.21\n";
...
const UTIL_GO = [
  "package tlvschema",
  "",
  "// Variable-width big-endian unsigned decode. Most widths are straightforward;",
...
// ---------------------------------------------------------------------------
// A record schema whose declared field size exceeds the received payload (e.g.
// a record type only present in the default schema) slices payload[loc:a.Size]
// without clamping to the payload length → a slice out-of-range panic. The fix
// clamps the upper bound to the payload length. Self-contained here so a real
// `go test` goes red→green.
// ---------------------------------------------------------------------------
const SCHEMA_PKG = "internal/tlvschema";

const RECSCHEMA_GO = [
  "package tlvschema",
  "",
  "// Attr is one field of a record schema; Size is its cumulative",
  ...
  "\t\t// BUG: a.Size can exceed the received payload (a record type only",
  "\t\t// present in the default schema), so slicing to a.Size panics with",
  "\t\t// slice-out-of-range. The fix clamps the end to the payload length.",
  "\t\tend := a.Size",
...
async function setupSchemaModule(dir: string): Promise<void> {
  await write(dir, "go.mod", GO_MOD);
  await write(dir, `${SCHEMA_PKG}/recschema.go`, RECSCHEMA_GO);
  await write(dir, `${SCHEMA_PKG}/recschema_test.go`, RECSCHEMA_TEST_GO);
  await writeNoise(dir);
}
...
export const GO_TASKS: EvalTask[] = [
  ...
  {
    // Pre-fix code slices a schema field to its declared size without clamping
    // to the received payload, panicking (slice out of range) on short payloads.
    id: "tlv-attr-slice-panic",
    title: "Fix a slice-out-of-range panic on short payloads (go test)",
    ...
    prompt:
      "This is a Go module. Running `go test ./...` fails: decoding a record schema panics with a slice out-of-range error when the received payload is shorter than the schema's declared field sizes (this happens for record types only present in the default schema). Locate the offending decoder and fix it so `go test ./...` passes — a declared attribute that runs past the payload must be clamped to what was actually received, not cause a panic. Do not edit any _test.go files.",
    async check(dir) {
      if (!(await goAvailable(dir))) {
        return {
          pass: false,
      ...
      await write(dir, SCHEMA_PROBE_REL, RECSCHEMA_PROBE_GO);
      ...
      const src = await read(dir, `${SCHEMA_PKG}/recschema.go`);
      if (!/payload\[/.test(src)) {
