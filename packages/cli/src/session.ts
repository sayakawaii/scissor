...
/**
 * Resolve the sandbox for a session.
 *
 * The write-protection denylist is always on — it costs nothing and there is no
 * legitimate reason for the agent to rewrite git hooks or SSH keys. Hard
 * isolation (Docker/WSL) is opt-in, because it only works when the project's
 * toolchain is reachable inside the container, which is a per-project fact we
 * cannot assume.
 */
function resolveSandbox(workspaceRoot: string): SandboxPolicy {
  const backendEnv = (process.env.SCISSOR_SANDBOX ?? "none").trim().toLowerCase();
  const backend: SandboxBackend =
    backendEnv === "docker" || backendEnv === "wsl" ? backendEnv : "none";
  const image = process.env.SCISSOR_SANDBOX_IMAGE?.trim();
  const distro = process.env.SCISSOR_SANDBOX_DISTRO?.trim();
  return createSandboxPolicy(workspaceRoot, {
    backend,
    // Blocking the network only makes sense once commands are actually isolated;
    // asserting it under the "none" backend would be a claim we cannot enforce.
    network: backend !== "none" && process.env.SCISSOR_SANDBOX_NETWORK === "none" ? "none" : "full",
...
export interface Session {
  ...
  data: SessionData;
  /** The sandbox in force for this session. */
...
export async function createSession(opts: SessionOptions = {}): Promise<Session> {
  ...
  const sandbox = resolveSandbox(workspaceRoot);

  // Built after the tool set so the prompt's inventory is generated from the
  // tools this session really has, MCP tools included.
  ...
  return { agent, config, providerId, model, workspaceRoot, data, sandbox, mcp, tracer };
}
...
export async function persistSession(session: Session): Promise<void> {
  ...
  session.data.todos = session.agent.getTodos();
