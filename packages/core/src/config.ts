...
/**
 * Which speech engine to use; "auto" picks the best local option per platform.
 * "windows-stream" keeps one recognizer running (live partials, instant turns);
 * "windows" is the simpler spawn-per-turn engine.
 */
export type VoiceProviderId = "auto" | "windows" | "windows-stream" | "openai" | "none";

/**
 * Voice I/O config (spoken conversation via `/voice`). Pure data — the audio
 * implementation lives entirely in the CLI (UI) layer; core only stores it.
 */
export interface VoiceConfig {
  /** Allow the `/voice` command at all (default true). */
  enabled?: boolean;
  /** Speech-to-text engine. "auto" → Windows System.Speech on win32. */
  stt?: VoiceProviderId;
  /** Text-to-speech engine for the agent's replies. "auto" → Windows SAPI. */
  tts?: VoiceProviderId;
  /** BCP-47 locale for recognition/synthesis, e.g. "en-US", "zh-CN". */
  locale?: string;
  /** Speak the agent's replies aloud in voice mode (default true). */
  speakReplies?: boolean;
  /** Seconds to wait for speech before a listen attempt gives up (default 10). */
