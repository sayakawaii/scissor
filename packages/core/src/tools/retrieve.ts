...
export const retrieveTool: Tool = {
  ...
  async run(args, ctx) {
    ...
    if (args.queries !== undefined && args.queries !== null) {
      const extra = coerceStringArray(args.queries, "queries");
      if (!extra) {
        return {
          content:
            "Error: 'queries' must be an array of strings (one phrasing per element). " +
            "For a single query use 'query'.",
          isError: true,
        };
      }
      queries.push(...extra);
    ...
    const requestedK = coerceNumber(args.k);
    const k = requestedK !== undefined && requestedK > 0 ? Math.min(requestedK, 20) : 8;
    try {
