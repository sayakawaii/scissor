...
import { coerceArray, coerceBoolean } from "./coerce.js";
import { checkMutation, displayPath, resolveInWorkspace } from "./paths.js";
...
export const writeFileTool: Tool = {
  ...
  async run(args, ctx) {
    ...
    const refusal = checkMutation(ctx, abs, target);
...
function collectEdits(args: Record<string, unknown>): { edits: EditSpec[]; error?: string } {
  ...
  if (raw !== undefined && raw !== null) {
    const items = coerceArray(raw, { field: "edits" });
    if (!items) {
      return {
        edits: [],
        error:
          "'edits' must be an array of { old_string, new_string, replace_all? } objects. " +
          "For one change use the top-level 'old_string'/'new_string' instead.",
      };
    }
    const edits: EditSpec[] = [];
    for (const item of items) {
      if (typeof item !== "object" || item === null) {
        return { edits: [], error: "each entry in 'edits' must be an object with 'old_string'." };
      }
      ...
      edits.push({ oldString, newString, replaceAll: coerceBoolean(o.replace_all) === true });
    }
  ...
  return {
    edits: [{ oldString, newString, replaceAll: coerceBoolean(args.replace_all) === true }],
  };
}
