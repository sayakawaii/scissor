...
// --- tlv-uint40-decode-bug (Scheme D, real-code grounded): the off-by-8 shift
...
{
  const t = task("tlv-uint40-decode-bug");
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "scissor-uint40-"));
  await t.setup!(dir);
  const utilPath = path.join(dir, "src/service/codec/schema/util.js");

  const before = await t.check(dir, "");
  assert.equal(before.pass, false, "tlv-uint40 fails before the fix");
  ...
  const after = await t.check(dir, "");
  assert.equal(after.pass, true, "tlv-uint40 passes after the correct fix");
